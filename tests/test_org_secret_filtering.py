...
class CompletionProneSecret:
    """Secret-like object that raises if completion-prone properties are accessed."""
...
class TestOrgSecretFiltering:
    ...
    @patch('src.clients.github.Github')
    def test_list_repo_secrets_avoids_completion_prone_fields(self, mock_github_class):
        """Test that filtering does not touch fields that force completion."""
        logger = Logger(verbose=False)
        ...
        assert result == ["REPO_SECRET_1", "REPO_SECRET_2"]
